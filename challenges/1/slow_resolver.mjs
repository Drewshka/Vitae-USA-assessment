...
async function getUser(context, id) {
let User = await context.redis.get(id);
if (User != undefined) return User

  else {
    User = await context.db.get(id);
    await context.redis.set(id, { id, name: User ? User.name : null });
    return context.redis.get(id);
  }
}

export async function loadUsers(context, ids) {
  const userPromises = ids.map(async id =>{
    const info = await getUser(context, id);
    return info.name == null ? null : info;
  }) 
